...
struct MovieReducer: Reducer {                           /// Відповідає за обробку дій та оновлення стану
    ...
    @Dependency(\.movieClientEffect) var movieClient    /// інєкція залежності для доступ до MovieClientEffect
        ...
        case .loadMovies:
               ...
               return .run { send in
                   await send(.moviesResponse(TaskResult {
                       try await movieClient.getMovies()  /// Новий спосіб отримання даних
                   }))
               }
